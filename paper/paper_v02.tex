...
\begin{figure}[tb]
    ...
    \caption{Stellar properties could be inferred incorrectly when assuming single star evolution if one does not consider the full multimodal posteriors. Main panel shows the $\chi^2$ values for fitting our $M = 3.5 \msun$ mass-gainer model at $X_c = 0.1$ with our entire grid of single star models. The true values and best fit are highlighted with lines. We add white contour lines at the 95\% confidence regime of a $\chi^2$ distribution with 1 degree of freedom.}
    \label{fig:chi2}
\end{figure}
